 ...
 *     https://github.com/bcoin-org/bcrypto/blob/master/test/drbg-test.js
 *     https://github.com/bcoin-org/bcrypto/blob/master/test/data/drbg-nist.json
 *   - hmac-drbg (Fedor Indutny, MIT License)
 *     https://github.com/indutny/hmac-drbg/blob/master/test/drbg-test.js
 *     https://github.com/indutny/hmac-drbg/blob/master/test/fixtures/hmac-drbg-nist.json
 */
...
describe('HMAC-DRBG', () => {
  describe('Basic functionality', () => {
    it('should create instance with default algorithm', () => {
      ...
      expect(drbg).toBeInstanceOf(HmacDrbg);
    });

    it('should create instance with specific algorithm', () => {
      const entropy = Buffer.from('totally random0123456789'.repeat(2));
      const drbg = new HmacDrbg('sha256', entropy);
      expect(drbg).toBeInstanceOf(HmacDrbg);
    });

    it('should support different hash algorithms', () => {
      const entropy = Buffer.alloc(64, 0x42); // Enough for sha512

      const algorithms = ['sha1', 'sha256', 'sha384', 'sha512'] as const;
      for (const alg of algorithms) {
        const drbg = new HmacDrbg(alg, entropy);
        const result = drbg.generate(16);
  ...
  describe('Error handling', () => {
    it('should throw on insufficient entropy', () => {
      const smallEntropy = Buffer.alloc(4); // Too small
      expect(() => {
        new HmacDrbg('sha256', smallEntropy);
      }).toThrow('Not enough entropy.');
    });

    it('should throw on invalid entropy type', () => {
      const drbg = new HmacDrbg();
      expect(() => {
        // @ts-ignore - testing runtime error
        drbg.instantiate('not a buffer');
      }).toThrow('Entropy must be a Buffer');
    });
    ...
    it('should throw when not initialized', () => {
      ...
      expect(() => {
        drbg.generate(32);
      }).toThrow('DRBG not initialized.');
    });

    it('should throw on reseed without initialization', () => {
      const drbg = new HmacDrbg();
  ...
  describe('HMAC-DRBG SHA256 compatibility', () => {
    function testDrbg(opt: {
      entropy: string;
      nonce?: string | null;
      pers?: string | null;
      size: number;
      ...
      const nonce = opt.nonce ? Buffer.from(opt.nonce, 'utf8') : undefined;
      const pers = opt.pers ? Buffer.from(opt.pers, 'utf8') : undefined;
      const size = opt.size;
      const expectedResult = Buffer.from(opt.res, 'hex');

      const drbg = new HmacDrbg('sha256', entropy, nonce, pers);
      const result = drbg.generate(size);

      expect(result).toEqual(expectedResult);
    }

    it('should match expected output with personalization', () => {
      testDrbg({
    ...
    it('should match expected output without personalization', () => {
      testDrbg({
        entropy: 'totally random0123456789',
        nonce: 'secret nonce',
        pers: null,
  ...
  describe('NIST test vectors', () => {
    for (const vector of getNISTVectors()) {
      it(`should pass NIST vector ${vector.name}`, () => {
        ...
        // Generate twice as per NIST test pattern
        for (let i = 0; i < vector.add.length; i++) {
          let add: Buffer | undefined;

          const additionalInput = vector.add[i];
          if (additionalInput !== null) {
            add = Buffer.from(additionalInput, 'hex');
          }
          const result = drbg.generate(expected.length, add);

          // Check only the final result in the chain
          const isFinalIteration = i === vector.add.length - 1;
          if (isFinalIteration) {
  ...
  describe('Reseeding', () => {
    it('should produce different output after reseed', () => {
      const entropy = Buffer.from(
        'totally random string with many chars that I typed in agony',
        'utf8',
      );
      const nonce = Buffer.from('nonce', 'utf8');
      const pers = Buffer.from('pers', 'utf8');

      const original = new HmacDrbg('sha256', entropy, nonce, pers);
      const reseeded = new HmacDrbg('sha256', entropy, nonce, pers);

      // Should produce same output initially
      expect(original.generate(32)).toEqual(reseeded.generate(32));

      // Reseed one instance
      reseeded.reseed(Buffer.from('another absolutely random string', 'utf8'));

      // Should produce different output after reseed
      expect(original.generate(32)).not.toEqual(reseeded.generate(32));
    });

    it('should handle reseed with additional input', () => {
      const entropy = Buffer.alloc(32, 0x42);
      const drbg = new HmacDrbg('sha256', entropy);

      const before = drbg.generate(32);

      drbg.reseed(Buffer.alloc(32, 0x43), Buffer.from('additional'));
      const after = drbg.generate(32);
  ...
  describe('Large outputs', () => {
    it('should generate large amounts of data correctly', () => {
      const entropy = Buffer.alloc(32, 0x42);
      const drbg = new HmacDrbg('sha256', entropy);

      const largeOutput = drbg.generate(1024);
      expect(largeOutput).toHaveLength(1024);

      // Should not be all zeros or all same values
      const first = largeOutput[0];
      const allSame = largeOutput.every((b) => b === first);
      expect(allSame).toBe(false);
    });
  });

  describe('Additional input handling', () => {
    it('should accept additional input in generate', () => {
      const entropy = Buffer.alloc(32, 0x42);
      const drbg = new HmacDrbg('sha256', entropy);

      const output1 = drbg.generate(32);

      // Reset with same entropy
      const drbg2 = new HmacDrbg('sha256', entropy);
      const output2 = drbg2.generate(32, Buffer.from('additional'));
...
function getNISTVectors(): NISTVector[] {
  return [
    {
      name: '0',
